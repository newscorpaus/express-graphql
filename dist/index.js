...
function graphqlHTTP(options) {
  ...
  return function (request, response) {
    ...
    return getGraphQLParams(request).then(function (graphQLParams) {
      ...
      var hash = sha256(JSON.stringify(schema) + JSON.stringify(source) + validationRules.toString());

      if (!cache[hash]) {
        cache[hash] = (0, _graphql.validate)(schema, documentAST, validationRules);
      }

      var validationErrors = cache[hash];
